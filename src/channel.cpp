    ...
    switch (message.type) {
        case IRCMessage::Type::PRIVMSG: {
            ...
            if(!sent) {
                ...
                if (tokens[0] == "zululending" && message.user == "hemirt") {
                ...
                else if (tokens[0] == "!joinchn" && message.user == "hemirt") {
                ...
                else if (tokens[0] == "!leavechn" && message.user == "hemirt") {
                    std::string msg = message.params;
                    ...
                    for(; i-- > 0;)
                    {
                        if(tokens[i] == "in" || tokens[i] == "IN" || tokens[i] == "iN" || tokens[i] == "In") {
                            break;
                        }
                    }
                    if(i == 0 || i + 1 >= tokens.size()) {
                        this->whisper("Usage \"!remindme [your message] in 20s 15h 10d 9m (the order does not matter, the number must be immediately followed by an identifier)\"", message.user);
                        return false;
                    }
                    int end = i;
                    ++i;
                    ...
                    if(seconds == 0) {
                        ...
                        this->whisper("Maximum amount of time is 7 days NaM", message.user);
                        return false;
                    }
                    else if(seconds < 0) {
                        this->whisper("Negative amount of time \xF0\x9F\xA4\x94, sorry I can't travel back in time (yet)", message.user);
                        return false;
                    }
                    ...
                    if(str.empty()) {
                        responsex = "You didnt set any message so here's your anonymous reminder eShrug";
                    }
                    
                    auto remindFunction = [owner = this->owner, user = message.user, responsex](const boost::system::error_code& er) -> void {
                        if(owner->channels.empty()) {
                            owner->joinChannel(owner->nick);
                        }
                        
                        owner->channels.at(owner->nick).whisper(responsex, user);
                    };
                    
                    auto t = new boost::asio::steady_timer(ioService, std::chrono::seconds(seconds));
                    t->async_wait(remindFunction);
                    std::string msg = message.user + ", reminding you in " + std::to_string(seconds) + " seconds";
                    if(j != 1) {
