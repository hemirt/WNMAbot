...
Response CommandsHandler::handle(const IRCMessage &message)
{
    ...
    */
    for(int j = 0; j < reply->elements; j++)
    {
        ...
        if(strncmp(reply->element[j]->str, "response", strlen(reply->element[j]->str)) == 0
            && j+1 < reply->elements)
        {
            std::cout << "dsaijdsaji" << reply->element[j+1]->str << std::endl;
...
Response CommandsHandler::addCommand()
{
